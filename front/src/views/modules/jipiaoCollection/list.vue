    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
                v-if="isAuth('jipiaoCollection','新增')"
                type="success"
                icon="el-icon-plus"
            ...
            &nbsp;
            <el-button
                v-if="isAuth('jipiaoCollection','删除')"
            ...
            &nbsp;
            <el-button
                v-if="isAuth('jipiaoCollection','报表')"
                type="success"
            ...
            <a style="text-decoration:none" class="el-button el-button--success"
               v-if="isAuth('jipiaoCollection','导入导出')"
               icon="el-icon-download"
               href="http://localhost:8080/mhwangshangdingpiao/upload/jipiaoCollectionMuBan.xls"
            >批量导入航班收藏数据模板</a>
            &nbsp;
            <el-upload
                v-if="isAuth('jipiaoCollection','导入导出')"
                ...
                :action="getActionUrl"
                :on-success="jipiaoCollectionUploadSuccess"
                :on-error="jipiaoCollectionUploadError"
            ...
            &nbsp;
            <!-- 导出excel -->
            <download-excel v-if="isAuth('jipiaoCollection','导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields"
                            name="jipiaoCollection.xls">
              <!-- 导出excel -->
              <el-button
                  type="success"
                  icon="el-icon-download"
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                  ...
                  v-if="isAuth('jipiaoCollection','查看')"
                  :data="dataList"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="jipiaoCollectionTypes"
                           header-align="center"
                           label="类型">
            <template slot-scope="scope">
              {{ scope.row.jipiaoCollectionValue }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('jipiaoCollection','查看')" type="success" icon="el-icon-tickets" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">详情
              </el-button>
              <el-button v-if="isAuth('jipiaoCollection','修改')" type="primary" icon="el-icon-edit" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">修改
              </el-button>
              <el-button v-if="isAuth('jipiaoCollection','删除')" type="danger" icon="el-icon-delete" size="mini"
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {//当前表
          tableName: 'jipiao_collection',//当前表表名,
          sumColum: 'jipiao_collection_number', //求和字段
          date: 'insert_time',//分组日期字段
          // string : 'jipiao_collection_name',//分组字符串字段
          // types : 'jipiao_collection_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
      ...
      //         groupColumn: "jipiao_collection_types",
      //// riqi :getMonthFormat(_this.echartsDate),//年月
      //     }
      //     this.$http({
    ...
    getDataList() {
      ...
      params['jipiaoCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


      this.$http({
        url: "jipiaoCollection/page",
        method: "get",
        params: params
      }).then(({data}) => {
        if (data && data.code === 0) {
    ...
    // 导入功能上传文件成功后调用导入方法
    jipiaoCollectionUploadSuccess(data) {
      let _this = this;
      _this.$http({
        url: "jipiaoCollection/batchInsert?fileName=" + data.file,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
          _this.$message({
    ...
    // 导入功能上传文件失败后调用导入方法
    jipiaoCollectionUploadError(data) {
