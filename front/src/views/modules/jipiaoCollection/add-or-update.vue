      ...
      <el-row>
        <el-col :span="12" v-if="sessionTable !='jipiao'">
          <el-form-item class="select" v-if="type!='info'" label="航班信息" prop="jipiaoId">
            <el-select v-model="ruleForm.jipiaoId" :disabled="ro.jipiaoId" filterable placeholder="请选择航班信息"
                       @change="jipiaoChange">
              <el-option
                  v-for="item in jipiaoOptions"
                  v-bind:key="item.id"
        ...
        <el-col :span="12" v-if="sessionTable !='jipiao' ">
          <el-form-item class="upload" v-if="type!='info' && !ro.jipiaoPhoto" label="航班照片" prop="jipiaoPhoto">
            <img style="margin-right:20px;" v-bind:key="index"
                 v-for="(item,index) in (jipiaoForm.jipiaoPhoto || '').split(',')" :src="$base.url+item" width="100"
                 height="100">
          </el-form-item>
          <div v-else-if="type=='info'">
            <el-form-item v-if="ruleForm.jipiaoPhoto" label="航班照片" prop="jipiaoPhoto">
              <img style="margin-right:20px;" v-bind:key="index"
                   v-for="(item,index) in (ruleForm.jipiaoPhoto || '').split(',')" :src="$base.url+item" width="100"
                   height="100">
            </el-form-item>
          </div>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item class="select" v-if="type!='info'" label="类型" prop="jipiaoCollectionTypes">
            <el-select v-model="ruleForm.jipiaoCollectionTypes" :disabled="ro.jipiaoCollectionTypes"
                       placeholder="请选择类型">
              <el-option
                  v-for="item in jipiaoCollectionTypesOptions"
          ...
          <div v-else-if="type=='info'">
            <el-form-item class="input" label="类型" prop="jipiaoCollectionValue">
              <el-input v-model="ruleForm.jipiaoCollectionValue"
...
export default {
  data() {
    return {
      ...
      ro: {
        jipiaoId: false,
        yonghuId: false,
        jipiaoCollectionTypes: false,
        insertTime: false,
      },
      ruleForm: {
        jipiaoId: '',
        yonghuId: '',
        jipiaoCollectionTypes: '',
        insertTime: '',
      },
      jipiaoCollectionTypesOptions: [],
      jipiaoOptions: [],
      yonghuOptions: [],
      rules: {
        jipiaoId: [
          {required: true, message: '航班不能为空', trigger: 'blur'},
          {
            pattern: /^[0-9]*$/,
            message: '只允许输入整数',
            trigger: 'blur'
  ...
  created() {
    ...
    //获取下拉框信息
    this.$http({
      url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=jipiao_collection_types`,
      method: "get"
    }).then(({data}) => {
      if (data && data.code === 0) {
        this.jipiaoCollectionTypesOptions = data.data.list;
      }
    });
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jipiaoCollection/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jipiaoCollectionCrossAddOrUpdateFlag = false;
                  this.parent.search();
