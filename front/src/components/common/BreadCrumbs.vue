<template>
  <el-breadcrumb class="app-breadcrumb" separator=">>"
                 style="height:40px;background-Color:var(--publicMainColor);border-Radius:20px;padding:0px 20px 0px 20px;box-Shadow:0px 0px 0px 5px  var(--publicSubColor);border-Width:0;border-Style:solid;border-Color:var(--publicSubColor);">
    <transition-group :style="2==1?'justifyContent:flex-start;':2==2?'justifyContent:center;':'justifyContent:flex-end;'" class="box"
                      name="breadcrumb">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect"
              style="color:rgba(255, 215, 0, 1)">{{ item.name }}</span>
